...
export async function generateMetadata({ params }: { params: Promise<{ route: string }> }): Promise<Metadata> {
  ...
  return {
    title: data.metaTitle + " | 简体中文",
    description: data.metaDesc,
    alternates: { canonical: `${BASE}/zh-cn/services/${data.slug}`, languages: { en: `${BASE}/services/${data.slug}`, 'zh-CN': `${BASE}/zh-cn/services/${data.slug}` } },
    openGraph: { title: data.metaTitle, description: data.metaDesc, url: `${BASE}/zh-cn/services/${data.slug}`, locale: "zh_CN" },
  };
}
