...
/* ── Driver-instruction options ─────────────────────────────────────── */
const DRIVER_OPTS = [
  { value: "",                                    key: "book_drv_none"   },
...
/* ── Section divider ── */
function SectionLabel({ label, optional, lang }: { label: string; optional?: boolean; lang: "en"|"ja"|"zh" }) {
  return (
    <div className="flex items-center gap-3 mb-6 sm:mb-8">
      <span className="w-5 h-px bg-[#c9a84c]" />
      <p className="text-[#c9a84c] text-[9px] tracking-[0.45em] uppercase">{label}</p>
      {optional && (
        <span className="text-[9px] tracking-[0.3em] text-white/25 uppercase border border-white/10 px-2 py-0.5">
          {t.book_optional[lang]}
        </span>
      )}
...
function FieldWrap({ label, required, optional, htmlFor, children }: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={htmlFor}
        className="text-[10px] tracking-[0.28em] text-white/45 uppercase flex items-center gap-2">
        {label}
        {required && <span className="text-[#c9a84c]">*</span>}
        {optional && <span className="text-white/20 text-[9px]">— opt.</span>}
      </label>
      {children}
    </div>
  );
...
/* ── Date / time field — clicking label calls showPicker() ── */
...
/* ── Stepper (editable number, hides spin arrows via globals.css) ── */
    ...
    <div className="flex items-center">
      ...
      <input type="number" value={value} min={min} max={max}
        ...
        className="stepper-input w-12 h-9 border-y border-white/20 bg-transparent text-white
                   text-sm text-center outline-none tracking-widest focus:border-[#c9a84c]/50 transition-colors" />
      <button type="button" onClick={() => onChange(clamp(value + 1))}
        className="w-9 h-9 flex items-center justify-center border border-white/20 text-white/55
                   hover:border-[#c9a84c] hover:text-[#c9a84c] transition-colors select-none text-base">
        +
...
function VehicleCard({ v, nameLabel, capLabel, exLabel, selected, onSelect, lang }: {
  ...
  return (
    ...
    >
      ...
      {/* Gold check badge */}
      {selected && (
        <div className="absolute top-2 right-2 z-10 w-5 h-5 rounded-full bg-[#c9a84c]
      ...
      {/* Image / placeholder */}
      <div className="bg-[#111] flex items-center justify-center h-28 sm:h-32">
        {v.img ? (
          <Image src={v.img} alt={nameLabel} width={200} height={100}
            className="object-contain max-h-24 w-auto" />
        ) : (
          /* "No preference" — abstract car outline */
          <svg className="w-14 h-14 text-white/[0.07]" fill="none" stroke="currentColor"
      ...
      <div className="px-3 py-3 flex-1">
        <div className="flex items-start justify-between gap-1">
          <span className="text-white text-[12px] sm:text-[13px] font-semibold leading-tight">
            {nameLabel}
          </span>
          <span className={`text-[11px] font-mono shrink-0 tabular-nums
            ${v.price === "+¥0" ? "text-white/35" : "text-[#c9a84c]"}`}>
            {v.price}
          </span>
        </div>
        {capLabel && (
          <p className="text-white/40 text-[10px] mt-1.5 leading-snug">{capLabel}</p>
        )}
        {exLabel && (
          <p className="text-white/25 text-[10px] mt-0.5">【{lang === "ja" ? "例" : "e.g."}】{exLabel}</p>
        )}
      </div>
    </label>
  );
      ...
      <main className="min-h-screen bg-[#0c0c0c] flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 py-28 text-center">
          ...
          <button onClick={() => {
            ...
            setName(""); setEmail(""); setPhone(""); setVehicle("none"); setDriverMsg("");
      ...
      <div className="bg-[#111111] py-14 sm:py-20 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-14 sm:space-y-16">
            ...
            {/* ═══ 1. ROUTE ═══ */}
            <div>
              <SectionLabel label={t.book_sec_route[lang]} lang={lang} />
            ...
            {/* ═══ 2. SCHEDULE ═══ */}
            <div>
              <SectionLabel label={t.book_sec_sched[lang]} lang={lang} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-8">
            ...
            {/* ═══ 3. PASSENGERS ═══ */}
            <div>
              <SectionLabel label={t.book_sec_pax[lang]} lang={lang} />
            ...
            {/* ═══ 4. VEHICLE ═══ */}
            <div>
              <SectionLabel label={t.book_sec_vehicle[lang]} optional lang={lang} />
            ...
            <div>
              <SectionLabel label={t.book_sec_driver[lang]} optional lang={lang} />
              {/* Note */}
              <p className="text-[11px] text-white/30 leading-[1.85] mb-5 border-l-2 border-[#c9a84c]/20 pl-3">
              ...
              <div className="relative max-w-sm">
                <select
                  ...
                  className="w-full appearance-none bg-[#111111] border-b border-white/20
                             focus:border-[#c9a84c] text-white text-[13px] tracking-wide
                             py-3 pr-8 outline-none transition-colors [color-scheme:dark] cursor-pointer"
                >
                  {DRIVER_OPTS.map(o => (
                ...
                <svg className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2
            ...
            {/* ═══ 6. CONTACT ═══ */}
            <div>
              <SectionLabel label={t.book_sec_contact[lang]} lang={lang} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-8">
              ...
              <div className="mt-8">
                <FieldWrap label={t.book_phone[lang]} optional htmlFor="book-phone">
                  {/* Dial code + number in one row */}
                  <div className="flex items-end gap-3">
                    <DialCodeSelect dialCode={dialCode}
                      onChange={(dial, code) => { setDialCode(dial); void code; }} />
                  ...
                  {/* WhatsApp toggle — only shows when phone is filled */}
                  {phone && (
                    <label className="mt-3 flex items-center gap-2.5 cursor-pointer group w-fit">
                      ...
                      {/* WhatsApp icon */}
                      <svg className={`w-4 h-4 transition-colors ${hasWhatsapp ? "text-[#25D366]" : "text-white/25"}`}
